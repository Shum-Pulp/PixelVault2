...
(function () {
  ...
  function beep(freq=440, dur=0.05){
    if(!actx) return;
    const o=actx.createOscillator(); o.type="square"; o.frequency.value=freq; o.connect(gain);
  ...
  function makeGhosts(spawns){
    ...
    for(let i=0;i<4;i++){
      ...
      gs.push({
        ...
        x:s.x, y:s.y, dir: {x:1,y:0},
        radius: 10, mode: "scatter", spawn: s,
        corner: corners[i]
      });
    }
    return gs;
  ...
  function updateHUD(){
    ...
    try{ localStorage.setItem("pacman_high", String(state.high)); }catch {}
  ...
  // Swipe directly on the maze to steer without covering the action.
  let swipeStart = null;
  ...
  ["pointerup", "pointercancel", "lostpointercapture"].forEach(eventName=>{
    canvas.addEventListener(eventName, ()=>{ swipeStart = null; });
  });
